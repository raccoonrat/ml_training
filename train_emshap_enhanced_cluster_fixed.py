"""
修复版本：使用Google Cluster Data训练EMSHAP增强模型
"""
...
from google_cluster_data_loader_fixed import GoogleClusterDataLoaderFixed
...
def train_emshap_with_cluster_data(model: EMSHAPEnhanced, features: np.ndarray, 
    ...
    history = trainer.train(
        ...
        num_epochs=config.get('num_epochs', 50),  # 减少轮数以快速测试
...
def evaluate_cluster_emshap_model(trainer: EMSHAPTrainer, test_features: np.ndarray, 
                                test_labels: np.ndarray, loader: GoogleClusterDataLoaderFixed,
                                config: dict) -> dict:
    """
    ...
    shapley_values = trainer.compute_shapley_values(
        test_features, 
        num_samples=config.get('shapley_samples', 200)  # 减少样本数以加快速度
    )
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='使用Google Cluster Data训练EMSHAP增强模型（修复版本）')
    ...
    # 设置日志
    setup_logging()
    logger.info("开始使用Google Cluster Data训练EMSHAP增强模型（修复版本）")
    ...
    config = {
        ...
        'gru_layers': 3,
        'dropout_rate': 0.2,
        'shapley_samples': 100,  # 进一步减少样本数以加快速度
